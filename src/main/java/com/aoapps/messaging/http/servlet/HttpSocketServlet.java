 ...
 * Copyright (C) 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2024, 2025, 2026  AO Industries, Inc.
    ...
    Map<Long, ? extends Message> getOutMessages() throws InterruptedException {
      ...
      synchronized (outQueue) {
        ...
        try {
          ...
          if (outQueueCurrentThread == currentThread) {
            outQueueCurrentThread = null;
          }
        }
      }
    }
            ...
            try {
              ...
              synchronized (socket.inQueue) {
                for (int i = 0; i < size; i++) {
                  // Get the sequence
                  Long seq = Long.valueOf(request.getParameter("s" + i));
                  // Get the type
